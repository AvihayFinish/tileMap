...
public class TargetMover: MonoBehaviour {
    ...
    IEnumerator MoveTowardsTheTarget() {
        for(;;) {
            ...
            if (enabled && !atTarget)
                //MakeOneStepTowardsTheTargetWithBFS();
                MakeOneStepTowardsTheTargetWithDijkstra();
        }
    }

    private void MakeOneStepTowardsTheTargetWithBFS() {
    ...
    private void MakeOneStepTowardsTheTargetWithDijkstra() {
        Vector3Int startNode = tilemap.WorldToCell(transform.position);
        Vector3Int endNode = targetInGrid;
